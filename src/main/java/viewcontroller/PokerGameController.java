...
public class PokerGameController {
     ...
     * @param startAmount the start amounto of chips
     */
    public PokerGameController(PokerGame gameView,
        String name, int startAmount) {

        this.gameView = gameView;

        human = new HumanPlayer(name, startAmount);
        ais = new AIPlayer[3];
        players = new Player[ais.length + 1];
        players[0] = human;
        for (int i = 0; i < ais.length; i++) {
            ais[i] = new AIPlayer("AI_" + (i + 1), startAmount);
            players[i + 1] = ais[i];
        }
